...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["18313535216@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["18313535216@s.whatsapp.net"] // Owner MrKing
		...
		case 'daftar':
					...
					if (isUser) return reply('𝐤𝐚𝐦𝐮 𝐬𝐮𝐝𝐚𝐡 𝐭𝐞𝐫𝐝𝐚𝐟𝐭𝐚𝐫 𝐝𝐢 𝐝𝐚𝐭𝐚𝐛𝐚𝐬𝐞 𝐁𝐨𝐭')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				case 'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ 𝐍𝐚𝐦𝐚 𝐁𝐨𝐭 : ${me.name}\n➽ 𝐎𝐰𝐧𝐞𝐫 𝐁𝐨𝐭 : @${ownerNumber}\n➽ 𝐩𝐫𝐞𝐟𝐢𝐱 : | ${prefix} |\n➽ 𝐓𝐨𝐭𝐚𝐥 𝐁𝐥𝐨𝐜𝐤 : ${blocked.length}\n➽ 𝐀𝐤𝐭𝐢𝐟 𝐒𝐞𝐣𝐚𝐤 : ${kyun(uptime)}\n\n➽ 𝐓𝐨𝐭𝐚𝐥 𝐏𝐞𝐧𝐠𝐠𝐮𝐧𝐚 : ${user.length} User\n➽ 𝐈𝐧𝐬𝐭𝐚𝐠𝐫𝐚𝐦 : 𝑩𝑬𝑳𝑼𝑴 𝑻𝑬𝑹𝑺𝑬𝑫𝑰𝑨\n➽ 𝐒𝐩𝐞𝐜𝐢𝐚𝐥 𝐓𝐡𝐚𝐧𝐤𝐬 𝐓𝐨 :\n\n➽ Allah SWT \n➽ MhankBarBar\n➽ Nurutomo\n➽ Monurios\n➽ MrKing`
            ...
            break  
				...
				case 'owner':
                 ...
                 client.sendMessage(from, '𝐉𝐢𝐤𝐚 𝐌𝐚𝐮 𝐒𝐚𝐯𝐞 𝐂𝐡𝐚𝐭 𝐀𝐣𝐚 𝐆𝐚𝐧 𝐎𝐰𝐧𝐞𝐫 𝐆𝐮𝐚 𝐅𝐚𝐤𝐛𝐨𝐲 𝐁𝐫𝐨 𝐇𝐚𝐭𝐢 𝐇𝐚𝐭𝐢 :)',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'leave': 
				    if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			    	anu = await client.groupLeave(from, `𝐁𝐲𝐞 𝐀𝐥𝐥 𝐌𝐞𝐦𝐛𝐞𝐫 *${groupMetadata.subject}*`, groupId)
	                break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    reply('𝐌𝐚𝐬𝐚𝐥𝐚𝐡 𝐭𝐞𝐥𝐚𝐡 𝐝𝐢 𝐥𝐚𝐩𝐨𝐫𝐤𝐚𝐧 𝐤𝐞 𝐨𝐰𝐧𝐞𝐫 𝐁𝐎𝐓, 𝐥𝐚𝐩𝐨𝐫𝐚𝐧 𝐩𝐚𝐥𝐬𝐮/𝐦𝐚𝐢𝐧𝟐 𝐭𝐢𝐝𝐚𝐤 𝐚𝐤𝐚𝐧 𝐝𝐢𝐭𝐚𝐧𝐠𝐠𝐚𝐩𝐢.')
                    break
                case 'request':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    client.sendMessage('18313535216@s.whatsapp.net', options, text, {quoted: mek})
                    reply('𝐑𝐄𝐐𝐔𝐄𝐒𝐓 𝐀𝐍𝐃𝐀 𝐓𝐄𝐋𝐀𝐇 𝐒𝐀𝐌𝐏𝐀𝐈 𝐤𝐞 𝐨𝐰𝐧𝐞𝐫 𝐁𝐎𝐓, 𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐬 𝐩𝐚𝐥𝐬𝐮/𝐦𝐚𝐢𝐧𝟐 𝐭𝐢𝐝𝐚𝐤 𝐚𝐤𝐚𝐧 𝐝𝐢𝐭𝐚𝐧𝐠𝐠𝐚𝐩𝐢.')
                    break
				case 'memeindo':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
